...
Версия: **Release 3.12 — Windows verified portable build**
...
Release 3.0–3.3 остаются замороженным функциональным baseline. Патчи 3.4–3.11 закрыли Windows launcher/runtime, Java, шрифт, ярлык и completion defects. **Release 3.12** исправляет неполное дерево проекта и ширину номеров строк, отвязывает масштаб обоев от размера окна, публикует проверяемый GitHub update feed и расширяет Java offline autocomplete. В интерфейсе временно показаны Java, Python, C++, JavaScript, HTML и CSS; реализации остальных языков сохранены внутри проекта и не удалены.
...
- Release 3.4 исправил Windows batch launcher; Release 3.5 закрыл UX/package-manager acceptance feedback; Release 3.6 закрыл native-window/CRLF/log defects; текущий verified candidate — Release 3.12;
...
Final Integration Gate поколения 3.x пройден. Текущий рабочий portable build — **Release 3.12** — выпускается целой папкой `Astra Studio`; подробности запуска — в `PORTABLE_README.md`, публикации обновлений — в `UPDATE_DISTRIBUTION.md`. Ручная owner-матрица по `TEST_REGISTRY_3_X.md` остаётся отдельным этапом sign-off и не подменяется автоматическими тестами.
