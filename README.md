...
Версия: **Release 3.11 — Windows verified portable build**
...
Release 3.0–3.3 остаются замороженным функциональным baseline. Патчи 3.4–3.6 закрыли Windows launcher/runtime и packaging defects. **Release 3.11** добавляет профиль Angel 404, глобальный Minecraft Rus и завершает Java workflow: согласованный JDK resolver, javac diagnostics без ложного installer prompt, корректный UTF-8 вывод кириллицы и расширенное offline autocomplete. `java -version` штатно пишет в stderr, но теперь проверяется по настоящему exit code без `NativeCommandError`. Текущий Windows gate содержит **252 теста**.
...
- Release 3.4 исправил Windows batch launcher; Release 3.5 закрыл UX/package-manager acceptance feedback; Release 3.6 закрыл native-window/CRLF/log defects; текущий verified candidate — Release 3.11;
...
Final Integration Gate поколения 3.x пройден. Текущий рабочий portable build — **Release 3.11** — выпускается целой папкой `Astra Studio`; подробности запуска — в `PORTABLE_README.md`, публикации обновлений — в `UPDATE_DISTRIBUTION.md`. Ручная owner-матрица по `TEST_REGISTRY_3_X.md` остаётся отдельным этапом sign-off и не подменяется автоматическими тестами.
