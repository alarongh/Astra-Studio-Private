...
Target build: **Release 3.12 — Windows verified portable build**
...
2. Use the Release 3.12 portable/source archive on Windows and extract it into a fresh normal writable folder. Do not reuse an older `.venv`.
...
Expected target in the Release 3.12 source tree is **259 collected tests**. The verified Windows result is **257 PASS / 2 SKIPPED / 0 FAILED** and 88 passing subtests. The gate includes native caption handling, Angel 404 bundled-font runtime, cyclic completion, desktop shortcut creation, Java UTF-8 output, isolated updater replacement and the Windows PowerShell stderr-version probe.
...
- if no defects remain, the current Release 3.12 acceptance candidate receives owner sign-off while Release 3.3 remains the frozen feature baseline;
