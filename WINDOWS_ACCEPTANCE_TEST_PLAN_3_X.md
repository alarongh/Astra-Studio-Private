...
Target build: **Release 3.11 — Windows verified portable build**  
...
2. Use the Release 3.11 portable/source archive on Windows and extract it into a fresh normal writable folder. Do not reuse an older `.venv`.
...
Expected target in the Release 3.11 source tree is **252 collected tests**. The verified Windows result is **250 PASS / 2 SKIPPED / 0 FAILED** and 88 passing subtests. It includes the native caption regression, Angel 404 bundled-font runtime, cyclic completion, real desktop shortcut creation, real Java UTF-8 Cyrillic output and a real Windows PowerShell stderr-version probe. The two reported skips are the explicitly POSIX-only executable-bit test and unavailable symlink creation for the current account; neither is counted as passed.
...
- if no defects remain, the current Release 3.11 acceptance candidate receives owner sign-off while Release 3.3 remains the frozen feature baseline;
